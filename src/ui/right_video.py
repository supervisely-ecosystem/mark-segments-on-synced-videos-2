...
card = Card(
    ...
    content=Container([preview, player, video_settings_container, sync_btn], direction="vertical"),
...
def draw_figures(value):
    ...
    if value is False:
        ...
        return
    key_id_map = KeyIdMap()
    ann_json = g.api.video.annotation.download(video_id=g.choosed_videos["right_video"].id)
    ...
    for fig in ann.figures:
        ...
        t_width, _ = font.getsize(text)

        cv2.putText(
            img,
            text,
            (int(col - t_width // 2 + 3), row + 3),
    ...
    img_path = os.path.join(f.pr_path, "right_player_regions.png")
